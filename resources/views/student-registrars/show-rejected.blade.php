...
@section("content")
  ...
  <div class="card card-secondary">
      <div class="card-header">
          <h3 class="card-title">List of Rejected Students</h3>
      </div>
      <div class="card-body">
            ...
            <div class="col-md-12 menu">
              <nav class="navecation" style="margin-left:-40px;margin-top:20px;">
                <ul id="navi">
                  ...
                  <li><a class="menu {{(request()->is('student-registrars*')) ? 'current' : '' }}" href="{{route('student-registrars.showapproved')}}">Rejected ({{$inactiveStatus}})</a></li>          
          ...
          <hr>
            ...
            <div style="float:left;margin-left:4px;">
              <select class="form-control select2bs4 select2-hidden-accessible deactivate_all" style="width:130px;"" data-select2-id="17" tabindex="-1" aria-hidden="true">
                ...
                <option data-select2-id="38" value="deactivateAll">Cancel</option>
              </select>
            </div>
        ...
        @foreach($data as $key => $user)
          <tr>
            ...
            <td>
              ...
              <form class="d-inline" id="#submitReject" action="{{ route('student-registrars.cancel', ['id' => $user->id]) }}" method="POST">
                @csrf 
              <input type="hidden" value="get" name="_method">
              <input type="submit" name="submit" id="reject" style="margin-top:5px;" class="btn btn-warning btn-sm" value="Cancel" onclick="return confirmHold()">
              </form>  
            </td>
          </tr>
        @endforeach 
        ...
        if(allVals.length <= 0)  {  
          alert("Please select row."); 
            ...
            if(check == true){  
                ...
                $.ajax({
                    url: '{{ url('student-registrarsCancelAll') }}',
                    type: 'get',
                      ...
                      if (data['success']) {
                           alert(data['success']);
                            $(".sub_chk:checked").each(function() {  
                              ...
                              window.location = "{{URL::route('student-registrars.showrejected')}}"; 
                            });
