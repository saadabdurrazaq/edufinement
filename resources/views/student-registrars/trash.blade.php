...
@section("content")
  ...
  <div class="card card-secondary">
      <div class="card-header">
          <h3 class="card-title">Trash Students</h3>
      </div>
      <div class="card-body">
            ...
            <div class="col-md-12 menu">
              <nav class="navecation" style="margin-left:-40px;margin-top:20px;">
                <ul id="navi">
                  ...
                  <li><a class="menu {{(request()->is('student-registrars*')) ? 'current' : '' }}" href="{{route('student-registrars.trash')}}">Trash ({{$countTrash}})</a></li>
                </ul>
              </nav>
            </div>
          </div>
          <hr>
            ...
            <div style="float:right;">
              <form action="{{route('student-registrars.trash')}}">
        ...
        @foreach($data as $key => $user)
          <tr>
            ...
            <td>
              ...
              <form class="d-inline" id="#submitRestore" action="{{route('student-registrars.restore', ['id' => $user->id])}}" method="POST">
                @csrf 
              <input type="hidden" value="get" name="_method">
              <input type="submit" name="submit" id="restore" style="margin-top:5px;" class="btn btn-success btn-sm" value="Restore" onclick="return confirmRestore()">
              </form> 

              <form class="d-inline" id="#submitDelete" action="{{route('student-registrars.delete-permanent', ['id' => $user->id])}}" method="POST">
                @csrf 
...
<script>
  ...
  document.getElementById('pagination').onchange = function() { 
    ...
    window.location = "{{URL::route('student-registrars.trash')}}?items=" + this.value; 
  }; 
      ...
      if(allVals.length <= 0)  {  
        alert("Please select row."); 
          ...
          if(check == true){  
              ...
              $.ajax({
                  url: '{{ url('student-registrarsRestoreAll') }}',
                  type: 'get',
                      ...
                      if (data['success']) {
                        alert(data['success']);
                          $(".sub_chk:checked").each(function() {  
                            ...
                            window.location = "{{URL::route('student-registrars.trash')}}"; 
                          });
      ...
      if(delVals.length <= 0)  {  
        alert("Please select row."); 
          ...
          if(checkDelRow == true){  
              ...
              $.ajax({
                  url: '{{ url('student-registrarsDeleteAll') }}',
                  type: 'get',
