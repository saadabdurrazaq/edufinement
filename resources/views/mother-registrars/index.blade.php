      ...
      <div class="card-body">
          <div class="row" style="margin-top:-20px;">
            <div class="col-md-12 menu">
              <nav class="navecation" style="margin-left:-40px;margin-top:20px;">
                <ul id="navi">
                  <li><a class="menu {{(request()->is('mother-registrars*')) ? 'current' : '' }}" href="{{route('mother-registrars.index')}}">All ({{$count}})</a></li>
                  ...
                  <li><a class="menu" href="{{route('mother-registrars.trash')}}">Trash ({{$countTrash}})</a></li>                   
                </ul>
              </nav>
            </div>
          </div>
          <hr>
            ...
            <div style="float:right;">
              <form action="{{route('mother-registrars.index')}}">
...
<script>
  ...
  document.getElementById('pagination').onchange = function() { 
    ...
    window.location = "{{URL::route('mother-registrars.index')}}?items=" + this.value; 
  }; 
        ...
        if(allVals.length <= 0)  {  
          alert("Please select row."); 
            ...
            if(check == true){  
                ...
                $.ajax({
                    url: '{{ url('mother-registrarsTrashAll') }}',
                    type: 'get',
                        ...
                        if (data['success']) {
                          alert(data['success']);
                            $(".sub_chk:checked").each(function() {  
                              ...
                              window.location = "{{URL::route('mother-registrars.index')}}"; 
                            });
